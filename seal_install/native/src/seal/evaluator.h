    ...
    */
    ...
    public:
        ...
        */
        ...
        // xyy add: test hoist
        inline void rotate_vector_inplace_hoist(
            Ciphertext &encrypted, std::vector<Ciphertext> &destination, std::vector<int> steps, const GaloisKeys &galois_keys,
            MemoryPoolHandle pool = MemoryManager::GetPool()) const
        {
            if (context_.key_context_data()->parms().scheme() != scheme_type::ckks)
            {
            ...
            rotate_internal_hoist(encrypted, destination, steps, galois_keys, std::move(pool));
        }
        ...
        */
        ...
        // xyy add: test hoist
        inline void rotate_vector_hoist(
            Ciphertext &encrypted, std::vector<int> steps, const GaloisKeys &galois_keys, std::vector<Ciphertext> &destination,
            MemoryPoolHandle pool = MemoryManager::GetPool()) const
        {
            // destination = encrypted;
            rotate_vector_inplace_hoist(encrypted, destination, steps, galois_keys, std::move(pool));
        }
        ...
        */
        ...
        void multiply_lazy(std::vector<Ciphertext> &encrypted1, std::vector<Ciphertext> &encrypted2, Ciphertext &destination, MemoryPoolHandle pool = MemoryManager::GetPool()) const;
        
        // xyy add: test for lazy reduction
        // void test_multiply_lazy(Ciphertext &encrypted1, Ciphertext &encrypted2, Ciphertext &destination, MemoryPoolHandle pool) const;
        /**
    ...
    private:
        ...
        // xyy add: test hoist
        void rotate_internal_hoist(
            Ciphertext &encrypted, std::vector<seal::Ciphertext> &destination, std::vector<int> steps, const GaloisKeys &galois_keys,
            MemoryPoolHandle pool) const;

        void apply_galois_inplace_hoist(
            Ciphertext &encrypted, std::vector<seal::Ciphertext> &destination, std::vector<std::uint32_t> galois_elt_steps, const GaloisKeys &galois_keys, MemoryPoolHandle pool) const;

        // xyy add : lazy reduction
        void barrett_reduce_lazy_accumulate(Ciphertext &destination, std::vector<unsigned __int128> &acc, util::RNSIter result, MemoryPoolHandle pool = MemoryManager::GetPool()) const;
        ...
        void switch_key_inplace_hoist(
            Ciphertext &encrypted, util::ConstRNSIter target_iter, std::vector<Ciphertext> &destination, std::vector<std::uint32_t> &galois_elt_steps, const KSwitchKeys &kswitch_keys, 
