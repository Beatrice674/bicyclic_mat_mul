...
namespace seal
{
    ...
    // xyy add:
    void Evaluator::barrett_reduce_lazy_accumulate(Ciphertext &destination, std::vector<unsigned __int128> &acc, RNSIter result, MemoryPoolHandle pool) const
    {
        auto &context_data = *context_.get_context_data(destination.parms_id());
        auto &parms = context_data.parms();
        size_t coeff_count = parms.poly_modulus_degree();
        size_t coeff_modulus_size = parms.coeff_modulus().size();
        size_t index = 0;

        SEAL_ITERATE(iter(parms.coeff_modulus(), result), coeff_modulus_size, [&](auto I) {

            auto modulus_value = get<0>(I);

            SEAL_ITERATE(iter(size_t(0), get<1>(I)), coeff_count, [&](auto J) {

                unsigned __int128 value = acc[index];
                uint64_t tmp[2];
                tmp[0] = static_cast<uint64_t>(value);         
                tmp[1] = static_cast<uint64_t>(value >> 64);   

                get<1>(J) = barrett_reduce_128(tmp, modulus_value);  
     ...
     // xyy add: for lazy reduction
    inline void dyadic_product_coeffmod_lazy(ConstCoeffIter operand1, ConstCoeffIter operand2, std::size_t coeff_count, std::vector<unsigned __int128> &result, size_t start_index)
    {
        SEAL_ITERATE(iter(operand1, operand2), coeff_count, [&](auto I) {
            ...
            // get the product in 128 bits
            // unsigned __int128 prod = (static_cast<unsigned __int128>(z[1]) << 64) | z[0];
            unsigned __int128 prod = static_cast<uint128_t>(get<0>(I)) * get<1>(I);
            result[start_index] += prod;
            start_index ++;
        });           
    }
    // xyy add: test for lazy reduction
    // void Evaluator::test_multiply_lazy(Ciphertext &encrypted1, Ciphertext &encrypted2, Ciphertext &destination, MemoryPoolHandle pool) const
    // {
    //     if (!(encrypted1.is_ntt_form() && encrypted2.is_ntt_form()))
    //     {
    ...
    //     {
    //         throw invalid_argument("encrypted1 and encrypted2 must have the same size");
    //     }
    ...
    //     // Prepare destination
    //     destination.resize(context_, context_data.parms_id(), dest_size);
    //     std::fill_n(destination.data(), destination.poly_modulus_degree() * dest_size * coeff_modulus_size, 0ULL);
    //     PolyIter destination_iter = iter(destination);
    //     // cache friendly tile size
    //     size_t tile_size = min<size_t>(coeff_count, size_t(256));
    //     size_t num_tiles = coeff_count / tile_size;

    //     // for lazy reduction
    //     RNSIter destination_0_iter(*destination_iter[0], coeff_count);
    //     RNSIter destination_1_iter(*destination_iter[1], coeff_count);
    //     RNSIter destination_2_iter(*destination_iter[2], coeff_count);

    //     // 初始化向量，并将其转化为iter形式
    //     vector<unsigned __int128> acc_0(coeff_modulus_size * coeff_count, 0);
    //     vector<unsigned __int128> acc_1(coeff_modulus_size * coeff_count, 0);
    //     vector<unsigned __int128> acc_2(coeff_modulus_size * coeff_count, 0);

    //     Ciphertext &encrypted_1 = encrypted1;
    //     Ciphertext &encrypted_2 = encrypted2;

    //     // Set up iterators for input ciphertexts
    //     PolyIter encrypted1_iter = iter(encrypted_1);
    //     PolyIter encrypted2_iter = iter(encrypted_2);
    ...
    //     // avoid the wrong pointer
    //     size_t index = 0;

    //     // Temporary buffer to store intermediate results
    //     // SEAL_ALLOCATE_GET_COEFF_ITER(temp, tile_size, pool);
    //     // SEAL_ALLOCATE_GET_COEFF_ITER(temp1, tile_size, pool);

    //     // Computes the output tile_size coefficients at a time
    //     // Given input tuples of polynomials x = (x[0], x[1]), y = (y[0], y[1]), computes
    //     // destination = (x[0] * y[0], x[0] * y[1] + x[1] * y[0], x[1] * y[1])
    ...
    //             // acc_2 += x[1] * y[1]
    //             dyadic_product_coeffmod_lazy(encrypted1_1_iter[0], encrypted2_1_iter[0], tile_size, acc_2, index);

    //             // acc_1 += x[1] * y[0]
    //             dyadic_product_coeffmod_lazy(encrypted1_1_iter[0], encrypted2_0_iter[0], tile_size, acc_1, index);
    //             // acc_1 += x[0] * y[1]
    //             dyadic_product_coeffmod_lazy(encrypted1_0_iter[0], encrypted2_1_iter[0], tile_size, acc_1, index);

    //             // acc_0 += x[0] * y[0]
    //             dyadic_product_coeffmod_lazy(encrypted1_0_iter[0], encrypted2_0_iter[0], tile_size, acc_0, index);
    ...
    //     barrett_reduce_lazy_accumulate(destination, acc_0, destination_0_iter, pool);
    //     barrett_reduce_lazy_accumulate(destination, acc_1, destination_1_iter, pool);
    //     barrett_reduce_lazy_accumulate(destination, acc_2, destination_2_iter, pool);
    ...
    void Evaluator::multiply_lazy(std::vector<Ciphertext> &encrypted1, std::vector<Ciphertext> &encrypted2, Ciphertext &destination, MemoryPoolHandle pool) const
    {
        if (!(encrypted1[0].is_ntt_form() && encrypted2[0].is_ntt_form()))
        {
            throw invalid_argument("encrypted1 or encrypted2 must be in NTT form");
        }
        if (encrypted1.size() != encrypted2.size())
        ...
        // Extract encryption parameters.
        auto &context_data = *context_.get_context_data(encrypted1[0].parms_id());
        auto &parms = context_data.parms();
        size_t coeff_count = parms.poly_modulus_degree();
        size_t coeff_modulus_size = parms.coeff_modulus().size();
        size_t encrypted1_size = encrypted1[0].size();
        size_t encrypted2_size = encrypted2[0].size();
        ...
        {
            Ciphertext &encrypted_1 = encrypted1[i];
            Ciphertext &encrypted_2 = encrypted2[i];
        ...
        destination.scale() = encrypted1[0].scale() * encrypted2[0].scale(); 
        if (!is_scale_within_bounds(destination.scale(), context_data))
        {
            throw invalid_argument("scale out of bounds");
        }
    ...
    void Evaluator::transform_from_ntt_inplace(Ciphertext &encrypted_ntt) const
    {
        ...
        void Evaluator::apply_galois_inplace_hoist(
        Ciphertext &encrypted, vector<seal::Ciphertext> &destination, std::vector<uint32_t> galois_elt_steps, const GaloisKeys &galois_keys, MemoryPoolHandle pool) const
    {
        // Verify parameters.
        if (!is_metadata_valid_for(encrypted, context_) || !is_buffer_valid(encrypted))
        {
        ...
        if (parms.scheme() == scheme_type::bfv)
        {
            ...
            auto encrypted_iter = iter(encrypted);
            galois_tool->apply_galois(encrypted_iter[0], coeff_modulus_size, 1, coeff_modulus, temp);
            ...
            galois_tool->apply_galois(encrypted_iter[1], coeff_modulus_size, 1, coeff_modulus, temp);
        }
        else if (parms.scheme() == scheme_type::ckks || parms.scheme() == scheme_type::bgv)
        {
            ...
            // First transform encrypted.data(0)
            auto encrypted_iter = iter(encrypted);
            // galois_tool->apply_galois_ntt(encrypted_iter[0], coeff_modulus_size, 1, temp);

            // Copy result to encrypted.data(0)
            // set_poly(temp, coeff_count, coeff_modulus_size, encrypted.data(0));

            // Next transform encrypted.data(1)
            galois_tool->apply_galois_ntt(encrypted_iter[1], coeff_modulus_size, 1, temp);
        }
        else
        {
        ...
        switch_key_inplace_hoist(
            encrypted, temp, destination, galois_elt_steps,static_cast<const KSwitchKeys &>(galois_keys), galois_keys, pool);
    }
    ...
    void Evaluator::rotate_internal_hoist(
        Ciphertext &encrypted, vector<Ciphertext> &destination, vector<int> steps, const GaloisKeys &galois_keys, MemoryPoolHandle pool) const
    {
        auto context_data_ptr = context_.get_context_data(encrypted.parms_id());
        ...
        vector<uint32_t> galois_elt_steps;

        for (auto &step : steps)
        {
            if (step == 0)
            {
                // No rotation needed, just copy the ciphertext
                destination.push_back(encrypted);
                continue;
            }
            // Check if Galois key is generated or not.
            if (galois_keys.has_key(galois_tool->get_elt_from_step(step)))
            {
                galois_elt_steps.push_back(galois_tool->get_elt_from_step(step));
            }
            else
            {
                /* 分解steps的操作 */
                // Convert the steps to NAF: guarantees using smallest HW
                vector<int> naf_steps = naf(step);
                ...
                SEAL_ITERATE(naf_steps.cbegin(), naf_steps.size(), [&](auto naf_step) {
                    // We might have a NAF-term of size coeff_count / 2; this corresponds
                    // to no rotation so we skip it. Otherwise call rotate_internal.
                    if (safe_cast<size_t>(abs(naf_step)) != (coeff_count >> 1))
                    {
                        galois_elt_steps.push_back(galois_tool->get_elt_from_step(naf_step));
                    }
                });
            }
        }

        apply_galois_inplace_hoist(encrypted, destination, galois_elt_steps, galois_keys, std::move(pool));
        ...
        MemoryPoolHandle pool) const
    {
        ...
        // hoist version of switch_key_inplace
    void Evaluator::switch_key_inplace_hoist(
        Ciphertext &encrypted, ConstRNSIter target_iter, std::vector<Ciphertext> &destination, std::vector<uint32_t> &galois_elt_steps, const KSwitchKeys &kswitch_keys,
        const GaloisKeys &galois_keys, MemoryPoolHandle pool) const
    {
        ...
        // xyy add : create and initialization, prepare for reuse g^{-1}(a) and store in a matrix
        SEAL_ALLOCATE_GET_RNS_ITER(t_ntt_matrix, coeff_count, rns_modulus_size * decomp_modulus_size, pool);
        auto t_ntt_iter = iter(t_ntt_matrix);
        SEAL_ITERATE(iter(size_t(0)), rns_modulus_size, [&](auto I) {
            size_t key_index = (I == decomp_modulus_size ? key_modulus_size - 1 : I);
            // Multiply with keys and perform lazy reduction on product's coefficients
            SEAL_ITERATE(iter(size_t(0)), decomp_modulus_size, [&](auto J) {
                ...
                if ((scheme == scheme_type::ckks || scheme == scheme_type::bgv) && (I == J))
                {
                    galois_tool->apply_galois_ntt(target_iter[J], 1, t_ntt_iter[I * decomp_modulus_size + J]);
                }
                // Perform RNS-NTT conversion
                else
                {
                    ...
                    // NTT conversion lazy outputs in [0, 4q)
                    ntt_negacyclic_harvey_lazy(t_ntt, key_ntt_tables[key_index]);
                    galois_tool->apply_galois_ntt(t_ntt, 1, t_ntt_iter[I * decomp_modulus_size + J]);
                }
            });
        });

        // rotation for encrypted_copy (maybe several times)
        for(auto &galois_elt : galois_elt_steps)
        {
            SEAL_ALLOCATE_GET_RNS_ITER(temp, coeff_count, coeff_modulus_size, pool);
            // xyy add : prepare for galois steps
            Ciphertext encrypted_copy = encrypted;

            // check if galois_elt is valid
            if (!galois_keys.has_key(galois_elt))
            {
                throw invalid_argument("Galois key not present");
            }
            ...
            // xyy add : apply galois for b
            auto encrypted_iter = iter(encrypted_copy); 
            SEAL_ALLOCATE_GET_RNS_ITER(temp_b, coeff_count, coeff_modulus_size, pool);
            galois_tool->apply_galois_ntt(encrypted_iter[0], coeff_modulus_size, galois_elt, temp_b);
            set_poly(temp_b, coeff_count, coeff_modulus_size, encrypted_copy.data(0));

            // galois_elt for key_vector
            size_t kswitch_keys_index = GaloisKeys::get_index(galois_elt); 
            ...
            SEAL_ITERATE(iter(size_t(0)), rns_modulus_size, [&](auto I) 
            {
                ...
                SEAL_ITERATE(iter(size_t(0)), decomp_modulus_size, [&](auto J) {
                    ...
                    // SEAL_ALLOCATE_GET_COEFF_ITER(t_ntt, coeff_count, pool);
                    SEAL_ALLOCATE_GET_COEFF_ITER(t_ntt_copy, coeff_count, pool);
                    ConstCoeffIter t_operand;
                
                    // t_operand is g^{-1} and ntt from
                    // then apply the permutation
                    galois_tool->apply_galois_ntt(t_ntt_iter[I * decomp_modulus_size + J], galois_elt, t_ntt_copy);
                    t_operand = t_ntt_copy;
            ...
            PolyIter t_poly_prod_iter(t_poly_prod.get(), coeff_count, rns_modulus_size);
            SEAL_ITERATE(iter(encrypted_copy, t_poly_prod_iter), key_component_count, [&](auto I) {
                if (scheme == scheme_type::bgv)
                {
                    const Modulus &plain_modulus = parms.plain_modulus();
                    // qk is the special prime
            ...
            // xyy add : for next galois
            // SEAL_ITERATE(iter(size_t(0)), rns_modulus_size, [&](auto I) {

            //     SEAL_ITERATE(iter(size_t(0)), decomp_modulus_size, [&](auto J) {
            //         SEAL_ALLOCATE_GET_COEFF_ITER(t_ntt, coeff_count, pool);
            //         SEAL_ALLOCATE_GET_COEFF_ITER(t_ntt_ex, coeff_count, pool);
            //         galois_tool->apply_galois_ntt(t_ntt_iter_test[I * decomp_modulus_size + J], galois_elt, t_ntt);
            //         t_ntt_ex = t_ntt;
            //         galois_tool->apply_galois_ntt(t_ntt_ex, 1, t_ntt_iter[I * decomp_modulus_size + J]);

            //     });
            // });

            // xyy :add the result to destination
            destination.push_back(encrypted_copy);
        }
